...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['coinSupply', this, this.coinSupply, 1],
    ['getIdentity', this, this.getIdentity, 1]
  ];
  return methods;
};
 ...
 * @param {String} height (optional) Block height (this is an integer value but should be passed as string to client.coinsupply)
 * @param {Function} callback
 */
Bitcoin.prototype.coinSupply = function(height, callback) {
  var self = this;
  if (parseInt(height) > 0) {
    this.client.coinSupply(height, function(err, response) {
      if (err) {
        return callback(self._wrapRPCError(err));
      }
      callback(null, response.result)
    ...
    this.client.coinSupply(function(err, response) {
      if (err) {
        return callback(self._wrapRPCError(err));
      }
      callback(null, response.result)
 ...
 * https://wiki.verus.io/#!faq-cli/clifaq-02_verus_commands.md
 * @param {String} name - ID name or ID i-address
 * @param {Function} callback
 */
Bitcoin.prototype.getIdentity = function(name, callback) {
  var self = this;
  this.client.getIdentity(name, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result)
